...
export default function ProjectCard({ project }: { project: Project }) {
   return (
      <Modal>
         <Card className="w-[270px] p-0 h-[240px] flex-grow-0 flex-shrink-0 overflow-hidden bg-background z-10 relative rounded-3xl">
            <div>
                  ...
                  <div className="relative bg-background rounded-t-lg -top-10 px-4 pt-6">
                     ...
                     <div className="flex justify-between items-center mt-2 mb-4">
                        <div className="flex gap-2 md:gap-3">
                           {project.liveURL && (
                              <ViewProject url={project.liveURL} />
                           )}
                           {project.codeURL && (
                              <ViewCode url={project.codeURL} />
                           )}
                        </div>
                        <div className="flex">
                           {project.article && project.article.uid && (
                              <ReadBlueprints
                                 url={`/writings/${project.article.uid}`}
                              />
                           )}
                        </div>
                     </div>
