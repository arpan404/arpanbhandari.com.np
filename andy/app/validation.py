...
async def validate_json(request: Request):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail="Invalid JSON data in request")
    if not json_data:
        logger.error(
        ...
        raise HTTPException(
            status_code=400, detail="Empty JSON data in request")
    ...
    if not user_details:
        ...
        raise HTTPException(
            status_code=400, detail="User details missing in request")
    if not user_details.get("name"):
        logger.error(f"Empty name received from {request.client.host}")
        raise HTTPException(
            status_code=400, detail="Name missing in user details")
    ...
    if len(name.split()) > 4:
        ...
        raise HTTPException(
            status_code=400, detail="Name must have at most 4 words")
    if len(name) > 50:
        logger.error(
            f"Name received from {
    ...
    if not user_details.get("email"):
        ...
        raise HTTPException(
            status_code=400, detail="Email missing in user details")
...
def validate_message(request: Request, json_data):
    ...
    if not json_data.get("chat_uid"):
        ...
        raise HTTPException(
            status_code=400, detail="ChatUID missing in request")

    if not json_data.get("message"):
        logger.error(f"Empty message received from {client_ip}")
        raise HTTPException(
            status_code=400, detail="Message missing in request")
...
async def validate_chat_uid(json_data: dict):
    if not json_data.get("chat_uid"):
        ...
        raise HTTPException(
            status_code=400, detail="Chat UID missing in request")
    ...
    if json_data.get("chat_uid") not in user.assigned_chats:
        ...
        raise HTTPException(
            status_code=400, detail="Chat UID not assigned to user")
