...
def referrer_validator(request: Request):
    ...
    if not (
        referrer == "https://arpanbhandari.com.np"
        or referrer == "http://localhost:3000"
    ):
        print("Forbidden", referrer)
        logger.warning(
            f"Forbidden access attempt from {
...
async def message_api_data_validaror(request: Request, json_data):
    ...
    if len(message) > 1000:
        ...
        raise HTTPException(
            status_code=400, detail="Message must have at most 500 characters"
        )
